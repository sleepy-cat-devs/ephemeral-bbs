...
class Const:
    ...
    # --- アプリケーション設定 ---
    IS_DEBUG: Final[bool] = os.getenv("ENV") == "dev"  # デバッグモード
    DEBUG_AUTH_BYPASS: Final[bool] = (
        IS_DEBUG and os.getenv("DEBUG_AUTH_BYPASS", "false").lower() == "true"
    )  # Discord認証を実行しないフラグ
    ...
    CLIENT_SECRET: Final[str] = os.getenv(
        "DISCORD_CLIENT_SECRET",
        "",
    )  # Discordアプリケーションのクライアントシークレット
    GUILD_IDS: Final[list[str]] = os.getenv(
        "DISCORD_GUILD_ID",
        "",
    ).split(",")  # アクセスを許可するDiscordサーバーID
    REDIRECT_URL: Final[str] = os.getenv(
        "REDIRECT_URL",
        f"http://localhost:{PORT}/callback",
    )  # Discord認証のリダイレクトURL

    API_BASE_URL: Final[str] = "https://discord.com/api"  # Discord APIのベースURL
    AUTHORIZATION_URL: Final[str] = (
        f"{API_BASE_URL}/oauth2/authorize"  # Discordの認証URL
    )
    TOKEN_URL: Final[str] = f"{API_BASE_URL}/oauth2/token"  # Discord認証のトークンURL

    _scopes: Final[list[str]] = ["identify", "guilds"]  # 許可するスコープ
