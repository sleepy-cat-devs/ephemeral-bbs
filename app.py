...
PORT = int(os.getenv("PORT", 5000))  # 環境変数からポートを取得、デフォルトは5000
...
def callback():
    ...
    if not authorization_code:
        return render_template(
            "error.html", title="Error", message="Authorization code not found."
        )
    ...
    try:
        ...
        guilds_request = requests.get(
            f"{API_BASE_URL}/users/@me/guilds", headers=auth_headers
        )
        ...
        user_info_request = requests.get(
            f"{API_BASE_URL}/users/@me", headers=auth_headers
        )
        ...
        return render_template(
            "error.html", title="Error", message=f"An API error occurred: {e}"
        )
